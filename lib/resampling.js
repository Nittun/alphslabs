...
export function reconstructSeriesFromBlocks(initialClose, shuffledBlocks, originalCandles) {
  ...
  for (const block of shuffledBlocks) {
    for (let i = 0; i < block.candles.length; i++) {
      ...
      const openRatio = origClose !== 0 ? originalCandle.open / origClose : 1
      const highRatio = origClose !== 0 ? originalCandle.high / origClose : 1
      const lowRatio = origClose !== 0 ? originalCandle.low / origClose : 1
      ...
      const originalDate = originalCandles[dateIdx]?.date || originalCandle.date
